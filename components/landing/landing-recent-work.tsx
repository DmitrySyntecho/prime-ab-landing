...
/* Block 3 — Recent Work grid. Real behind-the-scenes Mux videos from each event.
   If a real video isn't available for a future entry, leave muxId empty and the
   card renders a "Video coming soon" placeholder instead of a play button. */
interface WorkVideo {
  id: string
  muxId: string
  stat: string
...
function WorkCard({ video }: { video: WorkVideo }) {
  ...
  const thumbnail = video.muxId
    ? `https://image.mux.com/${video.muxId}/thumbnail.jpg?time=5&width=800`
    : ""
      ...
      <div className="relative w-full" style={{ aspectRatio: "16/9" }}>
        {playing && video.muxId ? (
          <video
            ...
            src={`https://stream.mux.com/${video.muxId}.m3u8`}
            autoPlay
            controls
            playsInline
          />
          ...
          <>
            {thumbnail ? (
              ...
              /* Placeholder — real video unavailable */
              <div className="absolute inset-0 grid place-items-center bg-[#0a0818] text-white/40 text-[13px] font-semibold">
                Video coming soon
              </div>
            )}
    ...
    <section className="relative py-14 md:py-20">
      <div className="max-w-7xl mx-auto px-4">
        <div className="mb-9 md:mb-12 max-w-3xl">
          ...
          <h2 className="text-[30px] md:text-[44px] font-extrabold tracking-[-0.025em] leading-[1.06] text-white mb-3">
