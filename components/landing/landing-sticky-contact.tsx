...
export function LandingStickyContact() {
  ...
  // Mobile bar appears once the user scrolls past the hero (~520px).
  useEffect(() => {
    let raf = 0
    let pending = false
    const handle = () => {
  ...
  // Close desktop widget on outside click.
  useEffect(() => {
    if (!open) return
  ...
  return (
    <>
      ...
      {/* ===== Mobile bottom bar ===== */}
      <div
        className={`md:hidden fixed inset-x-0 bottom-0 z-50 px-3 pt-2 transition-transform duration-300 ${
          showMobile ? "translate-y-0" : "translate-y-[140%]"
        }`}
        style={{ paddingBottom: "calc(0.55rem + env(safe-area-inset-bottom))" }}
      >
        <div
          className="flex items-stretch gap-2 p-2 rounded-2xl border border-white/10 backdrop-blur-2xl"
          style={{ background: "rgba(8,10,18,0.9)", boxShadow: "0 -8px 32px -12px rgba(0,0,0,0.8)" }}
        >
          <button
            ...
            style={{
              background: "linear-gradient(135deg, #FF2D6F 0%, #FF5E3A 100%)",
              boxShadow: "0 12px 28px -8px rgba(255,45,111,0.6)",
            }}
          >
